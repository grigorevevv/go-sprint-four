...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	firstParam := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift)
	return firstParam * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	firstParam := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) / (height / cmInM)
	secondParam := walkingCaloriesWeightMultiplier*weight + firstParam*walkingSpeedHeightMultiplier*weight
	return secondParam * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	swimmingMSpeed := swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift
	return swimmingMSpeed * swimmingCaloriesWeightMultiplier * weight * duration
}
